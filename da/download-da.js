...
async function exportDeadlyCSVUnique() {
  ...
  const enemies = [];
  const sides = container.querySelectorAll('[id^="s"]');

  sides.forEach(side => {
    const sideEnemies = side.querySelectorAll(".e");
    sideEnemies.forEach(enemy => {
      const nameEl = enemy.querySelector(".e-name");
      const hpEl = enemy.querySelector(".e-hp");
      const dazeEl = enemy.querySelector(".tt-e-stat .tt-text");

      const name = nameEl ? nameEl.textContent.trim() : "Unknown";
      const hp = hpEl ? parseInt(hpEl.childNodes[0].textContent.trim()) || 0 : 0;

      // Extract number after "Max Daze:"
      let daze = 0;
      if (dazeEl) {
        const dazeMatch = dazeEl.textContent.match(/Max Daze:\s*([\d.,]+)/);
        if (dazeMatch) daze = parseFloat(dazeMatch[1].replace(/,/g, ""));
      }

      enemies.push({ name, hp, daze });
    });
  });

  if (!enemies.length) {
    console.log("No enemies found in DOM.");
    return;
  }
  ...
  // Generate combined CSV (name, hp, daze)
  const csv = enemies
    .map(e => `"${e.name.replace(/"/g, '""')}","${e.hp}","${e.daze}"`)
    .join("\n");
  ...
  a.href = url;
  a.download = `690${versionNum < 10 ? `0` : ``}${versionNum}.csv`;
  a.click();
